...
<main>
    ...
    <nav>
        <a href="index.php?onglet=dashboard" <?= $onglet == 'dashboard' ? 'class="active"' : '' ?>>Tableau de Bord</a> |
        <a href="index.php?onglet=equipes" <?= $onglet == 'equipes' ? 'class="active"' : '' ?>>Equipes</a> |
    ...
    <section id="tableau-de-bord">
        <div>
            <div><strong><?= $stats['joueurs_attendus'] ?></strong><br>Joueurs attendus</div>
            <div><strong><?= $stats['avis_attente'] ?></strong><br>Avis en attente</div>
            <div><strong><?= $stats['taux_reussite'] ?>%</strong><br>Taux de réussite</div>
        </div>
            ...
            <tbody>
                <?php if (!empty($sessions)): ?>
                    ...
                    <tr>
                        ...
                        <td><?= $session['est_complete'] ? 'Complet' : 'Ouvert' ?></td>
                    </tr>
                    <?php endforeach; ?>
                <?php else: ?>
    ...
    <section id="equipes">
        <?php if (!empty($equipes)): ?>
            ...
            <article>
                ...
                <p>Session du : <?= $equipe['date_session'] ? date('d/m/Y', strtotime($equipe['date_session'])) : '—' ?></p>
                <table border="1" width="100%">
                    <tr><th>Nom</th><th>Prénom</th><th>Pseudo</th><th>Email</th><th>Téléphone</th></tr>
                    <tr>
                        <td><?= htmlspecialchars($equipe['nom'] ?? '—') ?></td>
                        <td><?= htmlspecialchars($equipe['prenom'] ?? '—') ?></td>
                        <td><?= htmlspecialchars($equipe['capitaine_pseudo'] ?? '—') ?></td>
                        <td><?= htmlspecialchars($equipe['email'] ?? '—') ?></td>
                        <td><?= htmlspecialchars($equipe['telephone'] ?? '—') ?></td>
                    </tr>
                </table>
            </article>
            <br>
    ...
    <section id="commentaires">
        <?php if (!empty($commentaires)): ?>
            ...
            <article>
                <h4>
                    <?= htmlspecialchars($avis['pseudo'] ?? 'Anonyme') ?>
                    <small><?= date('d/m/Y', strtotime($avis['date_publication'])) ?></small>
                </h4>
                <p>"<?= htmlspecialchars($avis['contenu']) ?>"</p>
    ...
    <section id="temps-equipes">
        ...
        <form action="index.php" method="POST">
            ...
            <label>Score (pts) : <input type="number" name="score" required></label>
        ...
        <table border="1" width="100%">
            ...
            <?php if (!empty($equipes)): ?>
                <?php foreach ($equipes as $eq): ?>
                    <?php if (!empty($eq['score_obtenu']) && $eq['score_obtenu'] > 0): ?>
                    <tr>
                        <td><?= htmlspecialchars($eq['nom_equipe']) ?></td>
                        <td><?= htmlspecialchars($eq['score_obtenu']) ?> pts</td>
                    </tr>
                    <?php endif; ?>
                <?php endforeach; ?>
            <?php else: ?>
                <tr><td colspan="2">Aucun score enregistré.</td></tr>
            <?php endif; ?>
        </table>
    </section>
    <?php endif; ?>
